...
final class TenantNovo extends Command
{
    ...
    public function handle(ProvisionamentoDeInstituicoes $provisionamento, ConteudoPorInstituicao $conteudo): int
    {
        ...
        // A ordem é a razão de este comando existir. Escrevê-la aqui, na saída, é mais barato
        // do que confiar em quem lembrou de abrir o runbook.
        //
        // E o primeiro passo NÃO é sempre o mesmo: o importador traz o legado e preserva os
        // ids, então só a PRIMEIRA instituição o recebe. As seguintes copiam o conteúdo dela,
        // com ids novos. Imprimir "rode o importador" para a segunda escola mandaria o
        // operador contra uma violação de chave primária.
        $dona = $conteudo->instituicaoComConteudo(exceto: $tenant->id);

        $semear = $dona === null
            ? "php artisan algorithmia:importar --tenant={$tenant->slug}"
            : "php artisan algorithmia:tenant:semear {$tenant->slug} --de={$dona->slug}";
